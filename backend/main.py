...
def save_scan_result(target, result, user_id):
    ...
    # If a user was deleted while an async scan is running, save report as unowned
    # instead of failing the whole job with a foreign key error.
    safe_user_id = user["id"] if user else None

    if user is None:
        logger.warning(
            f"Saving scan without user reference because user_id={user_id} no longer exists"
        )
...
async def run_wpscan_bruteforce(target: str, username: str, password_file: str | None = None):
    ...
    if not username:
        ...
        result["message"] = "Provide a WordPress username before running the password guess"
        return result

    if not os.path.exists(wordlist_path):
        result["error"] = f"Password file not found: {wordlist_path}"
        result["message"] = "Install rockyou or point to another password list"
        return result
    ...
    job_result = await asyncio.to_thread(bruteforce_job)

    if isinstance(job_result, Exception):
        message = str(job_result)
        if token:
            message = message.replace(token, "[REDACTED]")
        result["error"] = message or "WPScan bruteforce failed"
        result["message"] = "WPScan password guessing did not complete"
        return result

    result["executed"] = True
    result["raw_output"] = (job_result.stdout or "")[:4000]

    if job_result.returncode == 0:
        result["status"] = "completed"
        result["message"] = "WPScan finished"
    else:
        result["status"] = "no-match"
        result["message"] = "WPScan finished without a confirmed password"

    output_text = (job_result.stdout or "") + "\n" + (job_result.stderr or "")
    found_candidates = []
    for pattern in [r"password\s*[:=]\s*([^\s]+)", r"found\s*[:=]\s*([^\s]+)", r"(\S+)\s*\(user\)"]:
        import re
        for match in re.finditer(pattern, output_text, re.IGNORECASE):
            value = match.group(1).strip("'\"")
            if value and value not in found_candidates:
                found_candidates.append(value)

    if found_candidates:
        result["found_password"] = found_candidates[0]
        result["message"] = "Password found"
...
@app.post("/wpscan-bruteforce")
@limiter.limit("10/minute")
async def wpscan_bruteforce(
    request: Request,
    data: WpScanBruteforceRequest,
