...
// Render Scan Results
// =========================
async function startWordpressBruteforce(target, username) {
  const statusEl = document.getElementById("wp-bruteforce-status");
  const buttonEl = document.getElementById("wp-bruteforce-btn");

  if (!statusEl) return;
  if (!username) {
    statusEl.innerHTML = "<span class='muted'>Enter a username first.</span>";
    return;
  }

  if (buttonEl) buttonEl.disabled = true;
  statusEl.innerHTML = "<span class='muted'>Starting WPScan password guessing…</span>";

  try {
    const { res, data } = await apiFetch("/wpscan-bruteforce", {
      method: "POST",
      body: JSON.stringify({
        target,
        username,
    ...
    if (!res.ok) {
      throw new Error(data.detail || "WPScan bruteforce failed");
    }

    if (data.found_password) {
      statusEl.innerHTML = `<span style="color:#22c55e">Password found: ${esc(data.found_password)}</span>`;
      showToast("WPScan discovered a password candidate");
    } else if (data.status === "completed") {
      statusEl.innerHTML = "<span class='muted'>WPScan finished without a confirmed password.</span>";
    } else {
      statusEl.innerHTML = `<span class='muted'>${esc(data.message || data.error || "WPScan finished")}</span>`;
    }
  } catch (err) {
    statusEl.innerHTML = `<span style="color:#ef4444">${esc(err.message || "Bruteforce failed")}</span>`;
  } finally {
...
function renderScanResult(data) {
  ...
  const wpScanFindings = Array.isArray(data.wpscan?.findings) ? data.wpscan.findings : [];
  const dirsearchFindings = Array.isArray(data.dirsearch_scan?.findings) ? data.dirsearch_scan.findings : [];
  const wpUsernames = Array.isArray(data.wpscan?.usernames) ? data.wpscan.usernames : [];
  const wpUsernameHint = wpUsernames[0] || "";
  const wpTargetValue = String(data.target || data.host || "");
  const wpTargetSafe = wpTargetValue.replace(/'/g, "\\'");
  ...
  out.innerHTML = `
    <section class="scan-result-shell">
      ...
      <div class="result-panels">
        ...
        <div class="result-card">
          <h3>� WordPress Exposure</h3>
          <p><b>WordPress:</b> ${data.wpscan?.is_wordpress || data.dirsearch_scan?.is_wordpress ? "✅ Detected" : "❌ Not detected"}</p>
          <p><b>WPScan Executed:</b> ${data.wpscan?.executed ? "✅ Yes" : "❌ No"}</p>
          <p><b>Dirsearch Executed:</b> ${data.dirsearch_scan?.executed ? "✅ Yes" : "❌ No"}</p>
          <p><b>WPScan Vulns:</b> ${esc(data.wpscan?.total_vulnerabilities ?? 0)}</p>
          <p><b>WPScan Users:</b> ${esc(data.wpscan?.user_count ?? 0)}</p>
          <p><b>Dirsearch Paths:</b> ${esc(data.dirsearch_scan?.total_paths ?? 0)}</p>
          <p><b>Status:</b> ${esc(data.wpscan?.error || data.dirsearch_scan?.error || data.wpscan?.skipped_reason || data.dirsearch_scan?.skipped_reason || "OK")}</p>
          ${Array.isArray(data.wpscan?.usernames) && data.wpscan.usernames.length ? `<p><b>Found Users:</b> ${esc(data.wpscan.usernames.join(", "))}</p>` : ""}
        </div>
        ...
        <div class="result-card wide">
          <h3>🔐 WPScan Password Guessing</h3>
          <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center; margin-bottom:8px;">
            <input id="wp-bruteforce-user" value="${esc(wpUsernameHint)}" placeholder="username" style="padding:8px; border-radius:6px; border:1px solid #475569; background:#0f172a; color:#fff; min-width:220px;" />
            <button id="wp-bruteforce-btn" type="button" onclick="startWordpressBruteforce('${wpTargetSafe}', document.getElementById('wp-bruteforce-user').value)">Start Guessing</button>
          </div>
          <div id="wp-bruteforce-status" class="muted">Use the discovered username and the local rockyou wordlist.</div>
        </div>
        <div class="result-card wide">
          <h3>🧰 WPScan Findings</h3>
          ${wpScanFindings.length
            ? wpScanFindings.map(item => `
              <div class="mini-result">
                <b>${esc(item.component_type || "component")} · ${esc(item.component || "wordpress")}</b>
                <span>${esc(item.title || "WordPress vulnerability")}</span>
                ${item.fixed_in ? `<small>Fixed in: ${esc(item.fixed_in)}</small>` : ""}
                ${item.reference ? `<small>Reference: ${esc(item.reference)}</small>` : ""}
              </div>
            `).join("")
            : "<p class='muted'>No WPScan findings returned</p>"
          }
        </div>
        <div class="result-card wide">
          <h3>🗂️ Dirsearch WordPress Paths</h3>
          ${dirsearchFindings.length
            ? dirsearchFindings.map(item => `
              <div class="mini-result">
                <b>${esc(item.path || "Path")}</b>
                <span>${esc(item.status_code ? `HTTP ${item.status_code}` : "Discovered")}</span>
                ${item.evidence ? `<small>${esc(item.evidence)}</small>` : ""}
              </div>
            `).join("")
            : "<p class='muted'>No dirsearch WordPress paths returned</p>"
          }
        </div>
